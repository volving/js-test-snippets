                ...
                z.onFullfilledArray.push(function () {
                    setTimeout(function () {
                        try {
                            ...
                            doResolve(promise2, dex, resolve, reject);
                        } catch (e) {
                            reject(e)
                        }
                    });
...
function doResolve(promise, dex, resolve, reject) {  //这里的resolve可是 "正宗的"
    // promise must != dex
    if (promise === dex) {
    ...
    let isUsed;
    let type = typeof dex;
    if (dex !== null && (type === "object" || type === "function")) {
        try {
            let then = dex.then;
            if (typeof then === "function") {
                then.call(dex, function (dex_) {
                    if (isUsed) return;
                    isUsed = true;
                    doResolve(promise, dex_, resolve, reject)
                }, function (e) {
                    if (isUsed) return;
                    isUsed = true;
                    reject(e);
